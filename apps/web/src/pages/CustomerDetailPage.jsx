...
function MeasurementTile({ label, value, unit }) {
  return (
    <div className="p-4 rounded-xl" style={{ background: 'var(--gray-50)', border: '1px solid var(--gray-200)' }}>
      <p className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: 'var(--gray-400)' }}>{label}</p>
      <p className="text-base font-extrabold" style={{ color: 'var(--gray-800)', fontFamily: 'var(--font-heading)' }}>
...
export default function CustomerDetailPage({ customer, onDeleteCustomer, onNavigate, onRefresh }) {
  ...
  if (!customer) {
    return (
      <div className="max-w-lg mx-auto p-12 text-center card space-y-4 animate-fade-in">
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5 animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="text-xl font-extrabold" style={{ color: 'var(--gray-900)', fontFamily: 'var(--font-heading)' }}>
                {customer.fullName}
              </h2>
              <span
            ...
            <div className="mt-2 flex flex-wrap gap-4">
              ...
              {(isAdmin || isFullDetail) && customer.phoneNumber && (
                <span className="flex items-center gap-1.5 text-sm" style={{ color: 'var(--gray-600)' }}>
                  <Phone className="w-3.5 h-3.5" style={{ color: 'var(--gray-400)' }} />
                  {customer.phoneNumber}
                </span>
              )}
              {customer.createdAt && (
                <span className="flex items-center gap-1.5 text-sm" style={{ color: 'var(--gray-500)' }}>
                  <Calendar className="w-3.5 h-3.5" style={{ color: 'var(--gray-400)' }} />
                  Registered {new Date(customer.createdAt).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}
                </span>
              )}
            </div>
          </div>
      ...
      <div className="card overflow-hidden">
        {/* Category Tabs */}
        <div className="px-5 py-4" style={{ borderBottom: '1px solid var(--gray-100)', background: 'var(--gray-25)' }}>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            {existingCategories.length > 0 && (
              <button
                ...
                className="text-xs font-semibold px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5 self-start"
                style={{
        ...
        <div className="p-5 space-y-5">
          ...
          {!latestMeasurement ? (
            <div className="py-10 text-center rounded-xl" style={{ background: 'var(--gray-50)', border: '1px solid var(--gray-200)' }}>
            ...
            <div className="space-y-4">
              ...
              {latestMeasurement.notes && (
                <div className="p-4 rounded-xl" style={{ background: 'var(--gray-50)', border: '1px solid var(--gray-200)' }}>
                  <p className="text-xs font-bold uppercase tracking-wider mb-1.5 flex items-center gap-1.5" style={{ color: 'var(--gray-400)' }}>
                    <FileText className="w-3.5 h-3.5" /> Instructions & Notes
                  </p>
                  <p className="text-sm whitespace-pre-line" style={{ color: 'var(--gray-700)' }}>
                    {latestMeasurement.notes}
                  </p>
                </div>
              )}
          ...
          {showHistory && categoryMeasurements.length > 0 && (
            <div className="pt-4 space-y-2" style={{ borderTop: '1px solid var(--gray-100)' }}>
              ...
              {categoryMeasurements.map((m, idx) => (
                ...
                >
                  <div>
                    <span className="text-sm font-semibold" style={{ color: 'var(--gray-700)' }}>Version {m.version || categoryMeasurements.length - idx}</span>
                  ...
                  {(m.isLatest || m.is_latest) ? (
                    <span className="text-[10px] font-bold px-2.5 py-1 rounded-full" style={{ background: '#F0FDF4', color: '#16A34A', border: '1px solid #BBF7D0' }}>
                      Current
                    </span>
                  ) : (
                    <span className="text-[10px] font-bold px-2.5 py-1 rounded-full" style={{ background: 'var(--gray-100)', color: 'var(--gray-400)' }}>
                      Archived
                    </span>
                  )}
                </div>
      ...
      {deleteConfirm && (
        <div className="modal-overlay" onClick={() => setDeleteConfirm(false)}>
          <div className="modal-box p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-2.5 mt-6 pt-4" style={{ borderTop: '1px solid var(--gray-100)' }}>
              <button id="delete-cancel-modal" className="btn-secondary" onClick={() => setDeleteConfirm(false)}>Cancel</button>
              <button id="delete-confirm-modal" className="btn-danger" onClick={() => { onDeleteCustomer(customer.id, customer.fullName); setDeleteConfirm(false); }}>
